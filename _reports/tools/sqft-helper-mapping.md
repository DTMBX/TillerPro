...
Goal: Standardize a single sq-ft helper include and wire it into calculators
where a user otherwise has to manually compute area.

## Canonical Include

- Include: `_includes/components/c-sqft-helper.html`
- Purpose: Provide width/length (and optional height) inputs, compute floor area
  and optionally 4-wall area, then allow pushing that computed value into an
  existing calculator input.
- Key attributes:
  - `data-sqft-helper`
...
- Behavior:
  ...
  - “Use … area” buttons set target input values and dispatch `input`/`change`
    events.

## Notes

- Height is optional and only enabled where wall-area input is useful (cost
  estimator; waterproof helper UI). For calculators that only need floor area,
  height is hidden.
