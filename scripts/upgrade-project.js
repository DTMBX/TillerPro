...
async function upgradeSass() {
  ...
  for (const filePath of sassFiles) {
    ...
    if (importCount > 0) {
      ...
      summary.sassFilesConverted.push({ file: fileRelPath, importsReplaced: importCount, issues });
      console.log(`✓ Converted ${fileRelPath} (${importCount} import statement${importCount > 1 ? 's' : ''} updated)`);
      if (issues.length) {
...
async function upgradeDependencies() {
  ...
  async function updateDepSet(depSet, depType) {
    for (const [name, currVersion] of Object.entries(depSet)) {
      ...
      summary.depsUpdated.push({ name, oldVersion: currVersion, newVersion: latestVersion });
    }
  }
...
async function upgradeBuildScripts() {
  ...
  try {
    ...
    for (const fname of files) {
      ...
      content = content.replace(/^(\s*)(?:var|let|const)\s+([\w$]+)\s*=\s*require\(['"]([^'"]+)['"]\);/gm,
      ...
      content = content.replace(/^(\s*)(?:var|let|const)\s+\{\s*([^}]+)\s*\}\s*=\s*require\(['"]([^'"]+)['"]\);/gm,
      ...
      content = content.replace(/^(\s*)require\(['"]([^'"]+)['"]\);\s*$/gm,
        (_, indent, reqPath) => {
          requireReplacements++;
      ...
      while ((match = execPattern.exec(content)) !== null) {
        ...
        if (lowerCmd.includes('convert') || lowerCmd.includes('magick') || lowerCmd.includes('mogrify') || lowerCmd.includes('cwebp')) {
          // Attempt to parse the command for input/output files and options
          let inputFile = null, outputFile = null;
          let width = null, height = null, quality = null;
          ...
          if (inputFile && outputFile) {
            ...
            content = content.replace(fullMatch, `/* exec call replaced with Sharp: ${commandStr} */`);
          }
        }
      }
      ...
      if ((requireReplacements > 0 || execReplacements > 0) && content !== origContent) {
        ...
        summary.scriptsModified.push({ file: relPath, requiresFixed: requireReplacements, execReplaced: execReplacements });
        console.log(`✓ Refactored ${relPath} (require→import: ${requireReplacements}, exec→sharp: ${execReplacements})`);
      }
    }
  } catch {
        ...
        if (config.type === 'node') {
          ...
          if (config.program && summary.mainFileRenamed) {
            if (config.program.includes(summary.mainFileRenamed.oldName)) {
              config.program = config.program.replace(summary.mainFileRenamed.oldName, summary.mainFileRenamed.newName);
              changed = true;
            }
          }
        }
...
function gitCommitAll() {
  try {
    ...
    if (summary.depsUpdated.length || summary.depsRemoved.length || summary.depsAdded.length) msgLines.push(' - Update NPM dependencies');
    if (summary.scriptsModified.length) msgLines.push(' - Refactor build scripts (ESM & Sharp)');
    if (summary.launchConfigUpdated || summary.netlifyUpdated) msgLines.push(' - Update config files (VSCode/Netlify)');
...
async function main() {
  ...
  if (summary.sassFilesConverted.length) {
    ...
    report += '- **Sass Imports Converted**: No changes (no @import statements found or already migrated).\n';
  }
  // Dependency updates
  ...
  if (summary.scriptsModified.length) {
    report += '- **Build Scripts Refactored**: Modernized Node scripts (ESM imports, replaced image conversion with Sharp).';
    report += '\n  - Files:\n';
