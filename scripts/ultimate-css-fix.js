...
content = content.replace(/(--container-max:[^;]+;\n)(  width: min)/g, '$1\n$2');
content = content.replace(/(--container-max:[^;]+;\n)(  padding-inline:)/g, '$1\n$2');
...
content = content.replace(/(\})\n(\.ts-footer::after \{)/g, '$1\n\n$2');
content = content.replace(/@keyframes slide-in-right \{[\s\S]*?from \{/g, (match) =>
  match.replace(/from \{/, '\n  from {')
);
...
content = content.replace(/(background-color:[^;]+;[\s\n]*)(background: linear-gradient)/g, '$2');
content = content.replace(/(border-top-color:[^;]+;[\s\n]*)(border: [^;]+;)/g, '$2');
